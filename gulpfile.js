 ...
 * SVG path
 */
path.src.svg[0] = path.src.srcPath + path.src.svg[0];

path.dist.svg = path.dist.distPath + path.dist.svg;

path.watch.svg = [];
path.watch.svg[0] = path.src.svg[0];

/**
 * map.json path
 */
path.src.cMap[0] = path.src.srcPath + path.src.cMap[0];

path.dist.cMap = path.dist.distPath + path.dist.cMap;

path.watch.cMap = [];
path.watch.cMap[0] = path.src.cMap[0];

path.src.cVue[0] = path.src.srcPath + path.src.cVue[0];

path.dist.cVue = path.dist.distPath + path.dist.cVue;

path.watch.cVue = [];
path.watch.cVue[0] = path.src.cVue[0];
...
function cMap(){
    return gulp.src(path.src.cMap)
        .pipe(gulp.dest(path.dist.cMap))
		.pipe(browserSync.reload({stream: true}))
}
function cVue(){
    return gulp.src(path.src.cVue)
        .pipe(gulp.dest(path.dist.cVue))
		.pipe(browserSync.reload({stream: true}))
}
...
function watch(){
    ...
    gulp.watch(path.watch.cMap, cMap);
    gulp.watch(path.watch.cVue, cVue);
...
exports.default = gulp.series(
    gulp.parallel(clean),
    gulp.parallel(njk, scss, script, image, font,svg,cMap,cVue),
